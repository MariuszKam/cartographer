...
- **PF-1.6 SaveSession / Snapshot** — [`IMPLEMENTATION IN PROGRESS — VALIDATION PENDING`](PF_1_6_SAVE_SESSION_SNAPSHOT.md). Checkpoints A+B establish the operation-scoped read-only session lifecycle and immutable save snapshot; consumer migration and validation remain open.
